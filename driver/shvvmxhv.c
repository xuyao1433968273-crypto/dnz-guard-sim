    ...
    )
{
    ...
    //
    // ===== 老师工程细节：EPT violation -> 翻镜子 =====
    // EXIT_QUALIFICATION bit0-47 = faulting GPA；认人用 GUEST_CR3。
    // 换面后开 MTF，单步执行一条指令，下个 exit（MTF）收尾。
    // 注意：这里不能前移 RIP——出错的指令还没执行，恢复访问后要重跑。
    //
    ...
    //
    // ===== 老师工程细节：MTF 单步收尾 =====
    // 翻镜子换面后执行了一条指令，到这里恢复 EPT 项为无权限 + 关 MTF。
    // MTF 的 VM_EXIT_INSTRUCTION_LEN 是 0，RIP 已正确，不用动。
    //
    case EXIT_REASON_MONITOR_TRAP_FLAG:
    {
        DnzEptFinishFlip(VpData);
        return;
    }
